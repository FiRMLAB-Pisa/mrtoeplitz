"""A Torch-native MRI-NUFFT CUFINUFFT backend.

Upstream's CUFINUFFT operator allocates through CuPy, so a CUDA build that
holds its trajectory and its transfer as Torch tensors would have to convert
at every call. This subclasses that operator and its raw-plan wrapper,
replacing only the CuPy allocation and the SENSE loops with Torch operations;
backend registration, shape conventions, autodiff integration and the
guru-plan lifecycle stay owned upstream.

Register it with :func:`register_torch_cufinufft` before asking MRI-NUFFT for
``backend="cufinufft"``; :func:`mrtoeplitz.scalar_kernel` and its siblings do
so themselves.
"""
