...
def _torch() -> Any:
    try:
        ...
        raise ImportError(
            "Toeplitz kernels require Torch: pip install mrtoeplitz"
        ) from error
...
def _triton_matvecs() -> tuple[Any, Any]:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "operator; mrtoeplitz will not silently select a slower CUDA path."
        ) from error
...
def _cpu_packed_matvec(packed: Any, spectrum: Any, out: Any = None) -> Any | None:
    ...
    if (
        ...
        return None
    matvec = require("packed_hermitian_matvec")
...
class PolyphaseToeplitzKernel:
    ...
    def _phase(self, parity: tuple[int, ...], reference: Any) -> Any | None:
        """Return the linear phase standing for a component's half-index shift."""
        torch = _torch()
        if not any(parity):
            return None
...
class CompactToeplitzKernel:
    ...
    def _dense_values(self) -> Any:
        """Return the transfer over every grid location, zero where unsampled.
